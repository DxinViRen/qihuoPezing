...
//    [self.contentView addSubview:self.exchangeLabel];
    ...
    NSArray *viewArr = @[self.myIngolView,self.daySign];
    NSArray *viewNEwArr = @[self.myCollected,self.exchangeImgView];
    [self.myIngolView mas_makeConstraints:^(MASConstraintMaker *make) {
       
        make.top.equalTo(self.contentView).inset(10);
    }];
    
    [self.myCollected mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.myIngolView.mas_bottom).offset(10);
        make.left.equalTo(self.myIngolView);
    }];
...
//    NSArray *labelArr = @[self.signLabel,self.ingolLabel,self.collectLabel,self.exchangeLabel];
    ...
    [viewArr mas_distributeViewsAlongAxis:MASAxisTypeHorizontal withFixedSpacing:20 leadSpacing:15 tailSpacing:15];
    
    [viewArr mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(self.daySign.mas_width).multipliedBy(184/353.0);
        make.top.equalTo(self.contentView).inset(10);
    }];
    
    
    [viewNEwArr mas_distributeViewsAlongAxis:MASAxisTypeHorizontal withFixedSpacing:20 leadSpacing:15 tailSpacing:15];
    
    [viewNEwArr mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(self.daySign.mas_width).multipliedBy(184/353.0);
        make.centerY.equalTo(self.myCollected);
    }];
...
- (UIImageView *)myIngolView{
    if(!_myIngolView){
        ...
        _myIngolView.image = [UIImage imageNamed:@"qihuoxuetangD"];
    }
    return _myIngolView;
}
...
- (UIImageView *)myCollected{
    if (!_myCollected) {
        ...
        _myCollected.image = [UIImage imageNamed:@"MyCollectD"];
...
- (UIImageView *)daySign{
    if(!_daySign){
        ...
        _daySign.image  = [UIImage imageNamed:@"homeDayCal"];
...
- (UIImageView *)exchangeImgView
{
    if(!_exchangeImgView){
        ...
        _exchangeImgView.image = [UIImage imageNamed:@"HotNEwsD"];
